...
<table width="100%">
  ...
  <tbody>
    ...
    <tr <?php if($i%2 != 0) echo 'class="odd"'?> style="<?php if(!$rs->getIsActive()) echo 'background:#cdcdcd;'?>">
      <td><?php echo ++$i?></td>
      <td>
      	<a href="<?php echo url_for('banner/edit?id='.$rs->getId())?>" title="Edit" class="action">
		        ...
		        <?php } else { ?>
		        	 	<?php echo image_tag('/u/b/'.$rs->getPath(), array('style'=>'max-width:300px;max-height:150px;')); ?>
				<?php } ?>
				</a>
      </td>
	  <td>
		<?php if($rs->getMobileImg()){ ?>
			<a href="<?php echo url_for('banner/edit?id='.$rs->getId())?>" title="Edit" class="action">
				<?php echo image_tag('/u/b/'.$rs->getMobileImg(), array('style'=>'max-width:300px;max-height:150px;')); ?>
			</a>
		<?php } ?>
      </td>
      <td>
